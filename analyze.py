...
txt_paths, pids, fnames = [], [], []
n = 0
for pid,fname in db.items():
  n += 1

  
  txt_path = os.path.join('data', 'txt', fname)
  ...
  if os.path.isfile(txt_path): # some pdfs dont translate to txt
    ...
    if len(txt) > 1000 and len(txt) < 500000: # 500K is VERY conservative upper bound
      ...
      pids.append(pid)
      fnames.append(fname)
      print("read %d/%d (%s) with %d chars" % (n, len(db), fname, len(txt)))
    else:
      print("skipped %d/%d (%s) with %d chars: suspicious!" % (n, len(db), fname, len(txt)))
  else:
...
v = TfidfVectorizer(input='content', 
        encoding='ascii', decode_error='replace', strip_accents='ascii', 
...
v.fit(train_corpus) # Learn vocabulary and idf from training set.


# transform
print("transforming %d documents..." % (len(txt_paths), ))
corpus = make_corpus(txt_paths)
X = v.transform(corpus) # Transform documents to document-term matrix.
print(v.vocabulary_)
print(X.shape)

# write full matrix out
out = {}
out['X'] = X # this one is heavy! (document-term matrix)
